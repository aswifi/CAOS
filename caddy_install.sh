...
#       Description: Caddy v1.0.5 Final Install
#       Version: 2.0.0
#       Author: Toyo&Teddysun
#       Modified by aswifi
#=================================================
...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://github.com/aswifi/CAOS/raw/main/init.d/caddy_redhat -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://github.com/aswifi/CAOS/raw/main/init.d/caddy_other -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
